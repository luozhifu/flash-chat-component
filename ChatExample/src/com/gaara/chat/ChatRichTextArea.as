package com.gaara.chat
{
	import flash.display.Graphics;
	import flash.display.Shape;
	import flash.display.Sprite;
	import flash.events.Event;
	import flash.text.engine.ElementFormat;
	import flash.text.engine.FontDescription;
	import flash.text.engine.GroupElement;
	import flash.text.engine.TextBlock;
	import flash.text.engine.TextLine;
	import flash.text.engine.TextLineMirrorRegion;
	
	/**
	 *  @author:Gaara
	 *  2012-7-11
	 *  图文混排
	 **/
	public class ChatRichTextArea extends Sprite
	{
		/** 元素格式 **/
		public var format:ElementFormat;
		
		/** 记录初始位置 **/
		public var offsetY:int = 18;
		
		/** 行距 **/
		public const space:int = 10;
		
		public function ChatRichTextArea()
		{
			var fontDescription:FontDescription = new FontDescription("Arial");
			format = new ElementFormat(fontDescription);
			format.fontSize = 12;
			format.color = 0xd8e5c6;
			
			this.mouseEnabled = false;
		}
		
		
		/**
		 *  功能:添加文本
		 *  参数:
		 **/
		public function appendText(preGroup:GroupElement,groupE:GroupElement):void
		{
			var textBlock:TextBlock = new TextBlock;
			textBlock.content = preGroup;
			
			//创建前段
			var textLine:TextLine = textBlock.createTextLine (null, width);
			
			for each (var tlm:TextLineMirrorRegion in textLine.mirrorRegions) 
			{
				//用此变量来作为下划线标志
				if(tlm.element.userData){
					var shape:Shape = new Shape();
					var g:Graphics = shape.graphics;
					g.lineStyle(1,0xd8e5c6);
					g.moveTo(tlm.bounds.left, tlm.bounds.bottom+1);
					g.lineTo(tlm.bounds.left+tlm.bounds.width, tlm.bounds.bottom+1);
					textLine.addChild(shape);
				}
			}
			
			textLine.y = offsetY;
			addChild(textLine);
			
			//保存前辍的宽度
			var preX:int = textLine.width;
			
			//必须要new TextBlock,否则鼠标事件什么的全部失效
			textBlock = new TextBlock;
			textBlock.content = groupE;
			//第一次创建减去前辍宽度
			var textLineE:TextLine = textBlock.createTextLine (null, width - preX);
			textLineE.x = preX;
			while (textLineE)
			{
				
				addChild(textLineE);
				textLineE.y = offsetY;
				offsetY += textLine.height+space;
			//	this.height = offsetY;
				
				textLineE = textBlock.createTextLine(textLineE, width);
			}
			
			this.dispatchEvent(new Event(Event.CHANGE));
		}
		
		/**
		 *  功能:清除内容
		 *  参数:
		 **/
		public function clear():void
		{
			while(numChildren > 0){
				var tl:TextLine = removeChildAt(0) as TextLine;
			}
			offsetY = 16;
			this.height = 0;//需要设置高度为0
			
			ChatTextAreaUtil.stopAndClearFaces();
			
			this.dispatchEvent(new Event(Event.CHANGE));
		}
	}
}